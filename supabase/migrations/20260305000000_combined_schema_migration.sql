  ...
  -- 5b. Add hide_setup_checklist to user_preferences
  -- Migration: 20260108000000_add_hide_setup_checklist_to_user_preferences.sql
  DO $$
  BEGIN
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.columns
  ...
  -- 5c. Convert dark_mode to theme_preference
  -- Migration: 20260203135440_convert_dark_mode_to_theme_preference.sql
  DO $$
  BEGIN
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.columns
  ...
  ALTER TABLE user_preferences ALTER COLUMN theme_preference SET DEFAULT 'system';
  ALTER TABLE user_preferences ALTER COLUMN theme_preference SET NOT NULL;
  DO $$
  BEGIN
    IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'user_preferences_theme_preference_check') THEN
  ...
  -- 5d. Add pay_cycle and transfer_view_mode to user_preferences
  -- Migration: 20260209000000_add_pay_cycle_to_preferences.sql
  ALTER TABLE user_preferences
  ADD COLUMN IF NOT EXISTS pay_cycle jsonb DEFAULT NULL;
  ALTER TABLE user_preferences
  ADD COLUMN IF NOT EXISTS transfer_view_mode text DEFAULT 'monthly';
  DO $$
  BEGIN
    IF NOT EXISTS (
      SELECT 1 FROM pg_constraint WHERE conname = 'user_preferences_transfer_view_mode_check'
    ) THEN
      ALTER TABLE user_preferences
      ADD CONSTRAINT user_preferences_transfer_view_mode_check
  ...
  -- 5e. Add tax_settings_configured to user_preferences
  -- Migration: 20260209000001_add_tax_settings_configured_to_preferences.sql
  ALTER TABLE user_preferences
